...
#if defined(NDEBUG)
# error "Danielcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2be4e76c2bbdfcb3db3a93bd7341df6d9863047b81d16d47bb7bbd00d856fab6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Danielcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Danielcoin Signed Message:\n";
...
{
    // Danielcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 500000); // Danielcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Danielcoin: 1 days
static const int64 nTargetSpacing = 60 * 60; // Danielcoin: 1 hour
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Danielcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Danielcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x72d5040ac3885fe1a0306e97bf258e6839482541c0ccf8bfdcd2131be27ce95d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "March 4th, 2021. Spacex's SN15 completes its first successful landing from 10km.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e3a0d836e0dc18c25fb39f627f056d0dad3f095b273ff21fb89c3240cde17e8dd08c97844c0a18227a47ddd562fdf4d84ee7f822d06dfc0bbddff3e58dfbc428") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1628216688;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084712835;

        if (fTestNet)
        {
            block.nTime    = 1628216688;
            block.nNonce   = 386734629;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xdd5caa9e531f2b959abae5baed935fd6fd5af01a66adb87d93639adb840c9f2c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc3, 0xc6, 0xd3, 0xf6 }; // Danielcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DanielcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DanielcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DanielcoinMiner(CWallet *pwallet)
{
    printf("DanielcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("danielcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DanielcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DanielcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DanielcoinMiner, pwallet));
}
